...
impl ParallelIndex {
    ...
    pub async fn push(&self, hash: u64, dp: DataPosition) -> bool {
        let vec_i = hash % self.parallel;
        let mut set = self.get_link(vec_i).write().await;
        if set.is_moved() {
            return false;
        }
        self.size.fetch_add(set.push(hash, dp) as u64, Ordering::SeqCst);
        true
    }

    /// 查找数据
    /// 第一个返回值表示 数据是否被移动
    pub async fn find(&self, hash: u64) -> (bool, Option<DataPosition>) {
        let vec_i = hash % self.parallel;
    ...
    pub async fn del(&self, hash: u64) -> bool {
        let vec_i = hash % self.parallel;
        let mut set = self.get_link(vec_i).write().await;
        if set.is_moved() {
            return false;
        ...
        self.size.fetch_sub(set.del(hash) as u64, Ordering::SeqCst);
        true
    }
...
mod tests {
    ...
    pub fn test_parallel_index() {
        ...
        rt.block_on(async {
            ...
            assert_eq!(index.push(1, DataPosition::new(1, 1, 1)).await, true);
            assert_eq!(index.push(2, DataPosition::new(2, 2, 2)).await, true);
            assert_eq!(index.push(1, DataPosition::new(3, 3, 3)).await, true);
            assert_eq!(index.push(3, DataPosition::new(4, 4, 4)).await, true);

            assert_eq!(index.find(1).await, (true, Some(DataPosition::new(3, 3, 3))));
            assert_eq!(index.del(3).await, true);
            assert_eq!(index.find(3).await, (true, None));
